...
public sealed class MainWindow : Window
{
    ...
    public MainWindow(MainViewModel viewModel)
    {
        ...
        var root=new Grid{ColumnDefinitions=new ColumnDefinitions("292,*")};Content=root;
        var side=new Border{Padding=new Thickness(18,22,18,16),BorderThickness=new Thickness(0,0,1,0)};side.Bind(Border.BackgroundProperty,new DynamicResourceExtension("Sidebar"));side.Bind(Border.BorderBrushProperty,new DynamicResourceExtension("Line"));root.Children.Add(side);
        var sideGrid=new Grid{RowDefinitions=new RowDefinitions("Auto,Auto,Auto,*,Auto")};side.Child=sideGrid;
        var brandPanel=new StackPanel{Spacing=14,Margin=new Thickness(6,0,6,14)};
        var brand=new StackPanel{Orientation=Orientation.Horizontal,Spacing=12,VerticalAlignment=VerticalAlignment.Center};
        brand.Children.Add(BrandImage(48));var brandName=Design.Text("Valerius AI",24,true);brandName.Bind(TextBlock.ForegroundProperty,new DynamicResourceExtension("BrandTeal"));brand.Children.Add(brandName);brandPanel.Children.Add(brand);brandPanel.Children.Add(new Separator());sideGrid.Children.Add(brandPanel);

        var primaryRow=new Grid{ColumnDefinitions=new ColumnDefinitions("50,*"),ColumnSpacing=8,Margin=new Thickness(6,0,6,10)};
        var navButtons=new Dictionary<string,Button>();
        var libraryNav=IconButton("M3,5 C6,4 9,5 12,7 L12,20 C9,18 6,17 3,18 Z M21,5 C18,4 15,5 12,7 L12,20 C15,18 18,17 21,18 Z","Biblioteca");libraryNav.Classes.Add("sidebarPrimary");libraryNav.Height=48;libraryNav.MinHeight=48;libraryNav.MaxHeight=48;libraryNav.HorizontalAlignment=HorizontalAlignment.Stretch;if(libraryNav.Content is Avalonia.Controls.Shapes.Path libraryIcon)libraryIcon.Bind(Avalonia.Controls.Shapes.Shape.StrokeProperty,new DynamicResourceExtension("OnAccent"));libraryNav.Click+=(_,_)=>vm.Navigate("Biblioteca");navButtons["Biblioteca"]=libraryNav;primaryRow.Children.Add(libraryNav);
        var add=Design.Button("NOVO CHAT","Novo chat (⌘N)");add.Classes.Add("sidebarPrimary");add.Height=48;add.MinHeight=48;add.MaxHeight=48;add.FontSize=15;add.FontWeight=FontWeight.SemiBold;add.HorizontalAlignment=HorizontalAlignment.Stretch;add.Command=vm.NewChatCommand;Grid.SetColumn(add,1);primaryRow.Children.Add(add);Grid.SetRow(primaryRow,1);sideGrid.Children.Add(primaryRow);

        var searchIcon=new Avalonia.Controls.Shapes.Path{Data=Geometry.Parse("M10.5,4 A6.5,6.5 0 1 0 10.5,17 A6.5,6.5 0 1 0 10.5,4 M15.2,15.2 L21,21"),Width=18,Height=18,Stretch=Stretch.Uniform,StrokeThickness=1.6,StrokeLineCap=PenLineCap.Round,StrokeJoin=PenLineJoin.Round};searchIcon.Bind(Avalonia.Controls.Shapes.Shape.StrokeProperty,new DynamicResourceExtension("Muted"));
        var searchContent=new Grid{ColumnDefinitions=new ColumnDefinitions("Auto,*,Auto"),ColumnSpacing=8};searchContent.Children.Add(searchIcon);var conversationSearch=new TextBox{Watermark="Buscar conversas",Background=Brushes.Transparent,BorderThickness=new Thickness(0),Padding=new Thickness(2),MinHeight=34,FontSize=13};Avalonia.Automation.AutomationProperties.SetName(conversationSearch,"Filtrar conversas");conversationSearch.Bind(TextBox.TextProperty,new Binding(nameof(vm.ConversationSearch)){Mode=BindingMode.TwoWay});Grid.SetColumn(conversationSearch,1);searchContent.Children.Add(conversationSearch);var globalSearch=Design.Button("↗","Buscar em tudo");globalSearch.Classes.Add("nav");globalSearch.Padding=new Thickness(4);globalSearch.MinHeight=30;globalSearch.MinWidth=30;globalSearch.Click+=async(_,_)=>{vm.SearchQuery=vm.ConversationSearch;vm.Navigate("Buscar");await vm.SearchAsync();};Grid.SetColumn(globalSearch,2);searchContent.Children.Add(globalSearch);conversationSearch.KeyDown+=async(_,e)=>{if(e.Key==Key.Enter){e.Handled=true;vm.SearchQuery=vm.ConversationSearch;vm.Navigate("Buscar");await vm.SearchAsync();}};
        var searchNav=new Border{Child=searchContent,HorizontalAlignment=HorizontalAlignment.Stretch,MinHeight=46,CornerRadius=new CornerRadius(23),BorderThickness=new Thickness(1),Padding=new Thickness(12,5),Margin=new Thickness(6,0,6,14)};searchNav.Bind(Border.BackgroundProperty,new DynamicResourceExtension("Surface"));searchNav.Bind(Border.BorderBrushProperty,new DynamicResourceExtension("Line"));Grid.SetRow(searchNav,2);sideGrid.Children.Add(searchNav);

        var sections=new Grid{RowDefinitions=new RowDefinitions("Auto,Auto,Auto,*"),Margin=new Thickness(6,0)};Grid.SetRow(sections,3);sideGrid.Children.Add(sections);
        var folderHead=new Grid{ColumnDefinitions=new ColumnDefinitions("Auto,*,Auto,Auto"),ColumnSpacing=7,Margin=new Thickness(0,4,0,6)};folderHead.Children.Add(Design.Text("Pastas",16));var folderLine=new Separator{VerticalAlignment=VerticalAlignment.Center};Grid.SetColumn(folderLine,1);folderHead.Children.Add(folderLine);var newFolder=Design.Button("＋","Criar pasta");newFolder.Classes.Add("nav");newFolder.Padding=new Thickness(5);newFolder.MinHeight=30;newFolder.MinWidth=30;newFolder.FontSize=15;newFolder.Click+=async(_,_)=>{var name=await PromptAsync("Nova pasta","");if(!string.IsNullOrWhiteSpace(name))await vm.CreateFolderAsync(name);};Grid.SetColumn(newFolder,2);folderHead.Children.Add(newFolder);var folderToggle=Design.Button("⌄","Recolher pastas");folderToggle.Classes.Add("nav");folderToggle.Padding=new Thickness(4);folderToggle.MinHeight=30;folderToggle.MinWidth=30;Grid.SetColumn(folderToggle,3);folderHead.Children.Add(folderToggle);sections.Children.Add(folderHead);
        var folderList=new ItemsControl{ItemsSource=vm.Folders,Margin=new Thickness(0,0,0,12)};folderList.ItemTemplate=new FuncDataTemplate<Folder>((folder,_)=>folder is null?new Border():FolderListItem(folder));Grid.SetRow(folderList,1);sections.Children.Add(folderList);folderToggle.Click+=(_,_)=>{folderList.IsVisible=!folderList.IsVisible;folderToggle.Content=folderList.IsVisible?"⌄":"›";Avalonia.Automation.AutomationProperties.SetName(folderToggle,folderList.IsVisible?"Recolher pastas":"Expandir pastas");};

        var conversationHead=new Grid{ColumnDefinitions=new ColumnDefinitions("Auto,*,Auto"),ColumnSpacing=7,Margin=new Thickness(0,4,0,6)};conversationHead.Children.Add(Design.Text("Conversas",16));var conversationLine=new Separator{VerticalAlignment=VerticalAlignment.Center};Grid.SetColumn(conversationLine,1);conversationHead.Children.Add(conversationLine);var conversationToggle=Design.Button("⌄","Recolher conversas");conversationToggle.Classes.Add("nav");conversationToggle.Padding=new Thickness(4);conversationToggle.MinHeight=30;conversationToggle.MinWidth=30;Grid.SetColumn(conversationToggle,2);conversationHead.Children.Add(conversationToggle);Grid.SetRow(conversationHead,2);sections.Children.Add(conversationHead);
        ...
        list.SelectionChanged+=async(_,_)=>{if(list.SelectedItem is Conversation c && c.Id!=vm.SelectedConversation?.Id){vm.Navigate("Chat");await vm.OpenAsync(c);}};Grid.SetRow(list,3);sections.Children.Add(list);conversationToggle.Click+=(_,_)=>{list.IsVisible=!list.IsVisible;conversationToggle.Content=list.IsVisible?"⌄":"›";Avalonia.Automation.AutomationProperties.SetName(conversationToggle,list.IsVisible?"Recolher conversas":"Expandir conversas");};
        var footer=new StackPanel{Spacing=14,Margin=new Thickness(0,14,0,0)};footer.Children.Add(new Separator());
        ...
        var emptyComposerGrid=new Grid{ColumnDefinitions=new ColumnDefinitions("Auto,*,Auto")};var emptyAttach=Design.Button("＋","Adicionar arquivos ou imagens");emptyAttach.MinWidth=44;emptyAttach.Margin=new Thickness(4);emptyAttach.Click+=async(_,_)=>await ShowAttachmentMenuAsync();emptyComposerGrid.Children.Add(emptyAttach);Grid.SetColumn(emptyComposer,1);emptyComposerGrid.Children.Add(emptyComposer);var emptySend=Design.Button("↑","Enviar mensagem");emptySend.Classes.Add("primary");emptySend.MinWidth=44;emptySend.Margin=new Thickness(4);emptySend.Command=vm.SendCommand;Grid.SetColumn(emptySend,2);emptyComposerGrid.Children.Add(emptySend);var emptyComposerBorder=new Border{CornerRadius=new CornerRadius(16),BorderThickness=new Thickness(1),Padding=new Thickness(7),Child=emptyComposerGrid};emptyComposerBorder.Bind(Border.BackgroundProperty,new DynamicResourceExtension("Surface"));emptyComposerBorder.Bind(Border.BorderBrushProperty,new DynamicResourceExtension("Line"));empty.Children.Add(emptyComposerBorder);empty.Children.Add(AttachmentStrip());
        ...
        var composerGrid=new Grid{ColumnDefinitions=new ColumnDefinitions("Auto,*,Auto")};composerBorder.Child=composerGrid;
        ...
        var attach=Design.Button("＋","Adicionar arquivos ou imagens");attach.MinWidth=44;attach.Margin=new Thickness(4);attach.Click+=async(_,_)=>await ShowAttachmentMenuAsync();composerGrid.Children.Add(attach);Avalonia.Automation.AutomationProperties.SetName(composer,"Mensagem");composer.Bind(TextBox.TextProperty,new Binding(nameof(vm.Draft)){Mode=BindingMode.TwoWay});composer.Bind(IsEnabledProperty,new Binding(nameof(vm.CanEdit)));Grid.SetColumn(composer,1);composerGrid.Children.Add(composer);
        ...
        var send=Design.Button("↑","Enviar mensagem");send.Classes.Add("primary");send.MinWidth=44;send.FontSize=23;send.VerticalAlignment=VerticalAlignment.Bottom;send.Margin=new Thickness(4);send.Command=vm.SendCommand;Grid.SetColumn(send,2);composerGrid.Children.Add(send);
        var stop=Design.Button("■","Interromper geração");stop.MinWidth=44;stop.VerticalAlignment=VerticalAlignment.Bottom;stop.Margin=new Thickness(4);stop.Command=vm.StopCommand;Grid.SetColumn(stop,2);composerGrid.Children.Add(stop);composerArea.Children.Add(AttachmentStrip());
        ...
        SizeChanged+=(_,_)=>{var sidebarWidth=Bounds.Width<900?238:292;root.ColumnDefinitions[0].Width=new GridLength(sidebarWidth);main.Margin=new Thickness(Bounds.Width<900?18:30,8,Bounds.Width<900?18:30,16);empty.Width=Math.Max(340,Math.Min(650,Bounds.Width-sidebarWidth-90));};
    }
    ...
    private Button FolderListItem(Folder folder)
    {
        var button=Design.Button(folder.Name);button.Classes.Add("nav");button.FontSize=13;button.Padding=new Thickness(10,6);button.HorizontalAlignment=HorizontalAlignment.Stretch;button.HorizontalContentAlignment=HorizontalAlignment.Left;button.Click+=(_,_)=>vm.FilterByFolder(folder);
        var rename=new MenuItem{Header="Renomear pasta"};rename.Click+=async(_,_)=>{var name=await PromptAsync("Renomear pasta",folder.Name);if(name is not null)await Safe(()=>vm.RenameFolderAsync(folder,name));};
        var remove=new MenuItem{Header="Excluir pasta"};remove.Click+=async(_,_)=>{if(await ConfirmAsync("Excluir esta pasta?","As conversas serão preservadas e voltarão para a lista principal.","Excluir pasta"))await Safe(()=>vm.DeleteFolderAsync(folder));};
        var menu=new ContextMenu{ItemsSource=new[]{rename,remove}};button.ContextMenu=menu;
        button.AddHandler(PointerPressedEvent,(_,e)=>{if(e.GetCurrentPoint(button).Properties.PointerUpdateKind==PointerUpdateKind.RightButtonPressed){e.Handled=true;menu.Open(button);}},Avalonia.Interactivity.RoutingStrategies.Tunnel);return button;
    }
    ...
    private ItemsControl AttachmentStrip()
    {
        var list=new ItemsControl{ItemsSource=vm.Attachments,HorizontalAlignment=HorizontalAlignment.Left};list.ItemTemplate=new FuncDataTemplate<ChatAttachment>((item,_)=> {if(item is null)return new Border();var row=new StackPanel{Orientation=Orientation.Horizontal,Spacing=6};row.Children.Add(Design.Text((item.Kind=="image"?"Imagem · ":"Arquivo · ")+item.Name,12));var remove=Design.Button("×","Remover "+item.Name);remove.Padding=new Thickness(5,1);remove.MinHeight=26;remove.MinWidth=26;remove.Click+=(_,_)=>vm.RemoveAttachment(item);row.Children.Add(remove);var chip=new Border{Child=row,CornerRadius=new CornerRadius(12),Padding=new Thickness(9,4),Margin=new Thickness(0,3,6,3)};chip.Bind(Border.BackgroundProperty,new DynamicResourceExtension("Line"));return chip;});return list;
    }
    private async Task ShowAttachmentMenuAsync()
    {
        var dialog=Dialog("Adicionar ao contexto");var panel=new StackPanel{Margin=new Thickness(24),Spacing=10};panel.Children.Add(Design.Text("Adicionar ao contexto",21,true));panel.Children.Add(Design.Text("Escolha arquivos do Mac ou um documento que já está no Conhecimento.",13));var local=Design.Button("Escolher arquivos ou imagens do Mac…");local.HorizontalAlignment=HorizontalAlignment.Stretch;local.Click+=(_,_)=>dialog.Close("local");panel.Children.Add(local);
        if(vm.Documents.Count>0)panel.Children.Add(Design.Text("CONHECIMENTO",11,true));foreach(var document in vm.Documents.Where(x=>x.Status=="Pronto")){var button=Design.Button(document.Name);button.HorizontalAlignment=HorizontalAlignment.Stretch;button.HorizontalContentAlignment=HorizontalAlignment.Left;button.Click+=(_,_)=>dialog.Close(document.Id);panel.Children.Add(button);}dialog.Content=new ScrollViewer{Content=panel,MaxHeight=520};var choice=await dialog.ShowDialog<string?>(this);if(choice is null)return;
        try
        {
            if(choice=="local")
            {var files=await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions{Title="Adicionar ao contexto do chat",AllowMultiple=true,FileTypeFilter=[new FilePickerFileType("Documentos e imagens"){Patterns=["*.txt","*.md","*.csv","*.json","*.docx","*.pptx","*.xlsx","*.pdf","*.png","*.jpg","*.jpeg","*.webp"]}]});await vm.AddLocalAttachmentsAsync(files.Select(x=>x.Path.LocalPath));}
            else if(vm.Documents.FirstOrDefault(x=>x.Id==choice) is {} document)await vm.AddKnowledgeAttachmentAsync(document);
        }
        catch(Exception e){vm.Report(e,e is ModelException?e.Message:"Não foi possível adicionar este arquivo ao contexto.");}
    }
    ...
    private Border KnowledgePage()
    {
        ...
        var list=new ItemsControl{ItemsSource=vm.Documents};list.ItemTemplate=new FuncDataTemplate<KnowledgeDocument>((x,_)=>{var row=new Grid{ColumnDefinitions=new ColumnDefinitions("*,Auto"),Margin=new Thickness(0,5)};row.Children.Add(Design.Text($"{x?.Name}\n{x?.Status} · {x?.Size/1e6:0.0} MB",14));var remove=Design.Button("Excluir");remove.Click+=async(_,_)=>{if(x is not null&&await ConfirmAsync("Excluir do Conhecimento?","O arquivo original será preservado. Apenas o texto e o índice local serão removidos.","Excluir"))await Safe(()=>vm.DeleteDocumentAsync(x.Id));};Grid.SetColumn(remove,1);row.Children.Add(remove);return row;});body.Children.Add(list);return PageShell("Conhecimento","Documentos locais disponíveis para recuperação semântica no chat.",body);
    }
    private Border FilesPage()
    {
        var body=new StackPanel{Spacing=12,MaxWidth=820,HorizontalAlignment=HorizontalAlignment.Stretch};var list=new ItemsControl{ItemsSource=vm.Artifacts};list.ItemTemplate=new FuncDataTemplate<Artifact>((x,_)=>{var row=new Grid{ColumnDefinitions=new ColumnDefinitions("*,Auto"),ColumnSpacing=8};var open=Design.Button($"{x?.Name}  ·  {x?.Type}  ·  {x?.Size/1024.0:0} KB");open.HorizontalAlignment=HorizontalAlignment.Stretch;open.HorizontalContentAlignment=HorizontalAlignment.Left;open.Click+=(_,_)=>{if(x is not null)Process.Start(new ProcessStartInfo(x.Path){UseShellExecute=true});};row.Children.Add(open);var remove=Design.Button("Excluir");remove.Click+=async(_,_)=>{if(x is not null&&await ConfirmAsync("Excluir este arquivo?","O arquivo será removido deste computador e da Biblioteca.","Excluir"))await Safe(()=>vm.DeleteArtifactAsync(x));};Grid.SetColumn(remove,1);row.Children.Add(remove);return row;});body.Children.Add(list);return PageShell("Arquivos","Resultados reais criados pelo Valerius Work, validados e guardados localmente.",body);
    }
